...
<body>
		...
		<tr>
			<form search="product" method="POST">
				<input type="hidden" name="search" value="searchSubmit" /> <input
					...
					name="searchInput" size="15" /> <input type="submit" value="Search" />
			</form>
		</tr>
	...
	<table>
		<tr>
			<td valign="top"><b>Choose a category</b> <%@ page
					import="java.sql.*"%> <%-- -------- Open Connection Code -------- --%>
				<%
 ...
 %> <%-- display category table on the side --%> <%@ include
					file="/display_category/display_category.jsp"%>
				<%-- display category table on the side --%> <%-- -------- SELECT Statement Code -------- --%>
				<%
					...
					<%
						// Check if an insertion is requested
							if (action != null && action.equals("insertProduct")) {
								...
								rs = stmt
										.executeQuery("SELECT * FROM products WHERE sku = "
								...
								if (price < 0 || request.getParameter("p_name") == ""
										|| rs.next()) {
					...
					<table border="1">
						...
						<tr>
							<form action="product" method="POST">
								...
								<%-- Get the p_name --%>
								<td><input value="<%=rs.getString("p_name")%>"
									name="p_name" size="15" /></td>

								<%-- Get the sku --%>
								<td><input value="<%=rs.getInt("sku")%>" name="sku"
									size="15" /></td>

								<%-- Get the category_id --%>
								<td><input value="<%=rs.getInt("category_id")%>" name="cid"
									size="15" /></td>
				...
				</table> <a href="browsing"> Product Browsing Page </a> <br> <a
				href="browsing"> Category Page </a>
</body>
</html>